...
//        bfsList();
    ...
    static void dfsList(int v, boolean[] visited) {
        ...
        // 2. 자식 탐색
        List<Integer> childs = graph2[v];
        for(Integer n : childs) {
        	if(!visited[n]) {
        		dfsList(n,visited);
        	}
        }
    ...
    static void bfsList() {
        ...
        while(!q.isEmpty()) {
        	...
        	while(size-- > 0) {
        		...
        		// 5. 자식 탐색
        		List<Integer> childs = graph2[current];
    	...
    	System.out.printf("총 %d 번만에 탐색 완료%n", depth);
    }
